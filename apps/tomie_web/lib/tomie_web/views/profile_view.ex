  ...
  def bookmarklet(user) do
    ...
    javascript:(function() {
      var t;
        ...
        window.location = \"#{url}?url=\" + encodeURIComponent(window.location.href) + \"&content=\" + encodeURIComponent((t == '' ? '' : '> ' + t)) + \"&name=\" + encodeURIComponent(document.title) + \"&token=#{user.token}\";
    })();
    """
  end
end
